...
class ClassFiveController extends Controller
{
    public function read(Request $request)
    {
        ...
        try {
            $row = Special_students::where('institute_id', '=', $request->session()->get('institute_id'))->where('class_id', '=', 5)->firstOrFail();
            //print_r($row); exit;
            array_push($instRow, $row);
        } catch (\Exception $e) {
            // do task when error
        ...
        try {
            $row = Classwise_students::where('institute_id', '=', $request->session()->get('institute_id'))->where('class_id', '=', 5)->firstOrFail();
            array_push($instRow, $row);
        } catch (\Exception $e) {
            // do task when error
        ...
        try {
            $row = Repeater_students::where('institute_id', '=', $request->session()->get('institute_id'))->where('class_id', '=', 5)->firstOrFail();
            array_push($instRow, $row);
        } catch (\Exception $e) {
            // do task when error
